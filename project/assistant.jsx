// AI onboarding assistant. slide-in panel
const Assistant = ({ open, onClose, profile, state }) => {
 ...
 <span className="dot-pulse" style={{ background: "var(--text-3)" }}></span>
 <span className="dot-pulse" style={{ background: "var(--text-3)", animationDelay: "0.2s" }}></span>
 <span className="dot-pulse" style={{ background: "var(--text-3)", animationDelay: "0.4s" }}></span>
 <style>{`
 .dot-pulse { width: 6px; height: 6px; border-radius: 50%; animation: dotbounce 1.2s ease-in-out infinite; }
 @keyframes dotbounce { 0%, 80%, 100% { opacity: 0.3; transform: translateY(0); } 40% { opacity: 1; transform: translateY(-3px); } }
 `}</style>
 </div>
 )}
 </div>
...
const Bubble = ({ role, text }) => {
 ...
 <div style={{
 fontSize: 14, lineHeight: 1.6, color: "var(--text-1)", flex: 1, paddingTop: 2,
 }}>
 {text}
 </div>
 </div>
