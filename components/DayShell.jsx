...
import { DAY_DEFS, DAY_TASKS } from '../data';
import { Icon, Chrome, Page, Reflection } from './ui';
import DayContent from './DayContent';

export default function DayShell({ dayId, state, profile, onUpdate, onComplete, onBack, onOpenAssistant }) {
  ...
  return (
    <div className="app">
      ...
      <Page max={1180}>
        ...
        >
          <button onClick={onBack} className="chip" style={{ marginBottom: 24, cursor: 'pointer' }}>
            ...
            <div className="card" style={{ padding: 18 }}>
              ...
              <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px solid var(--border)' }}>
                <div className="progress-track">
                  <div className="progress-fill" style={{ width: `${(tasksDone / tasksDef.length) * 100}%` }} />
                </div>
              </div>
            </div>
...
function DayTransition({ dayId, def, profile, onContinue }) {
    ...
    >
      <div style={{ maxWidth: 640, textAlign: 'center' }}>
        ...
        <motion.h2
          ...
          className="display"
          style={{ fontSize: 'clamp(36px, 5.5vw, 56px)', margin: '0 0 22px' }}
        >
          {dayId === 5 ? (
        ...
        {dayId < 5 && (
          <motion.p
            ...
            style={{ color: 'var(--text-2)', fontSize: 16, lineHeight: 1.6, margin: '0 0 36px' }}
          >
            Tomorrow &rarr; <span style={{ color: next.color }}>{next.value}</span> · {next.theme}
          </motion.p>
        )}
